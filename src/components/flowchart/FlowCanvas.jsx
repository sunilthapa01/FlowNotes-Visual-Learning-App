...
import {
  ...
  Plus, Trash2, MousePointer2, Sparkles, Move
} from 'lucide-react'
...
export default function FlowCanvas({ activeNote, onSave }) {
  ...
  const handleZoom = (dir) => setZoom((z) => Math.min(2.5, Math.max(0.25, z + dir * 0.15)))
  ...
  const handleAddNode = useCallback((type, customTitle, customDesc) => {
    ...
    <div className="flex-1 flex flex-col relative overflow-hidden" style={{ minWidth: 0 }}>
      ...
      <div className="flex-1 flex overflow-hidden relative">
        ...
        <motion.div
          ...
          style={{ width: '130px', background: 'rgba(8,11,20,0.5)', backdropFilter: 'blur(14px)' }}
        >
          <div className="flex items-center gap-1.5 px-1 mb-2">
            <Sparkles size={11} style={{ color: '#4F8EF7' }} />
            <span className="text-xs font-bold text-[var(--text-muted)] uppercase tracking-wider" style={{ fontSize: '9px' }}>
              Add Node
            </span>
          </div>
          ...
          {NODE_TYPE_CATALOGUE.map((cat) => (
            <motion.button
              ...
              style={{
                background: hoveredType === cat.type ? `${cat.color}18` : 'rgba(255,255,255,0.03)',
                border: hoveredType === cat.type ? `1px solid ${cat.color}50` : '1px solid rgba(255,255,255,0.07)',
                boxShadow: hoveredType === cat.type ? `0 0 14px ${cat.color}20` : 'none',
              }}
            >
              {hoveredType === cat.type && (
                <motion.div
                  ...
                  style={{ background: `radial-gradient(ellipse at 30% 50%, ${cat.color}15 0%, transparent 70%)` }}
                />
              )}
              <div className="flex items-center gap-2 relative z-10">
                <motion.div
                  animate={hoveredType === cat.type
              ...
              <p className="text-xs mt-1 leading-tight relative z-10" style={{ color: 'var(--text-muted)', fontSize: '10px' }}>
                {cat.description}
              </p>
              <AnimatePresence>
                {hoveredType === cat.type && (
                  ...
                  >
                    ...
                    <span style={{ color: cat.color, fontSize: '9px', fontWeight: 600 }}>Click to add</span>
                  </motion.div>
                )}
              </AnimatePresence>
            </motion.button>
          ...
          >
            ...
            <span className="text-xs font-semibold" style={{ color: '#4F8EF7', fontSize: '10px' }}>Custom</span>
          </motion.button>
        </motion.div>
        ...
        >
          ...
          <div
            ...
            style={{ background: 'radial-gradient(ellipse at center, transparent 40%, rgba(8,11,20,0.55) 100%)' }}
          />
            ...
            {!hasNodes && (
              ...
              >
                <motion.div
                  ...
                  style={{ background: 'rgba(79,142,247,0.08)', border: '1px dashed rgba(79,142,247,0.3)' }}
                >
                  <Plus size={28} style={{ color: 'rgba(79,142,247,0.5)' }} />
                </motion.div>
                <div className="text-center">
                  <p className="text-sm font-medium" style={{ color: 'rgba(255,255,255,0.3)' }}>
                    {activeNote ? 'Click a node type to start your flow' : 'Create or select a note first'}
                  </p>
              ...
              <defs>
                ...
                {Object.entries(TYPE_COLOR).map(([type, color]) => (
                  <marker
                    ...
                    orient="auto"
                  >
                    <path d="M0,0 L0,6 L9,3 z" fill={color} fillOpacity="0.85" />
                  </marker>
                ))}
                {/* Default blue arrow */}
                ...
                return (
                  <g key={conn.id}>
                    ...
                    <path
                      ...
                      strokeDasharray={fromNode.type === 'decision' ? '6 3' : 'none'}
                      strokeLinecap="round"
